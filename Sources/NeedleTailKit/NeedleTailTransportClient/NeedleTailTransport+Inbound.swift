...
extension NeedleTailTransport {
    ...
    func doReadKeyBundle(_ keyBundle: [String]) async throws {
        ...
        guard let keyBundle = keyBundle.first else { throw NeedleTailError.cannotReadKeyBundle }
        guard let data = Data(base64Encoded: keyBundle) else { throw NeedleTailError.cannotReadKeyBundle }
        let buffer = ByteBuffer(data: data)
        let config = try BSONDecoder().decode(UserConfig.self, from: Document(buffer: buffer))
        keyBundleStore.keyBundle = config
    }
...
#if (os(macOS) || os(iOS))
        messenger.plugin.emitter.requestMessageId = messageId
#endif
    }
